...
function App() {
  ...
  // NUEVA FUNCIÓN: Abre la gestión y guarda el historial para el botón atrás
  const abrirGestion = () => {
    setVistaActiva('usuarios');
    window.history.pushState({ vista: 'usuarios' }, '', '');
  };

  // MODIFICADA: Ahora funciona bien devolviéndose desde cualquier pantalla
  const volverInicio = () => {
    setSucursal(null);
    setVistaActiva('inicio');
  ...
  if (!sucursal) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 relative overflow-x-hidden" style={premiumBackground}>
        
        {/* MODIFICADO: CONTENEDOR DE BOTONES RESPONSIVO */}
        <div className="absolute top-0 left-0 w-full p-4 sm:p-6 flex justify-center sm:justify-end gap-3 z-10 bg-black/20 sm:bg-transparent backdrop-blur-sm sm:backdrop-blur-none">
          {usuarioLogueado.rol === 'superadmin' && (
            <button onClick={abrirGestion} className="bg-[#FFB800] text-[#4A2511] hover:bg-yellow-400 px-4 py-2 rounded-xl font-bold shadow-lg transition-transform active:scale-95 text-sm sm:text-base">
              Usuarios
            </button>
          )}
          <button onClick={cerrarSesion} className="bg-red-500 text-white hover:bg-red-600 px-4 py-2 rounded-xl font-bold shadow-lg transition-transform active:scale-95 text-sm sm:text-base">
            Cerrar Sesión
          </button>
        </div>

        {/* MODIFICADO: mt-20 para que no choque con los botones en celular */}
        <h1 className="text-4xl md:text-5xl font-bold text-[#FFB800] mb-4 drop-shadow-xl tracking-wider text-center mt-20 sm:mt-0">
        ...
        {/* MODIFICADO: items-stretch en lugar de items-center para que ambas midan igual */}
        <div className="flex flex-col sm:flex-row gap-10 w-full max-w-4xl justify-center items-stretch pb-10">
          ...
          <button onClick={() => elegirSucursal('Sarchí')} className="flex-1 bg-white border-4 border-[#FFF0C2] hover:border-[#FFB800] rounded-3xl shadow-2xl p-6 flex flex-col items-center transition-all duration-300 hover:-translate-y-2 active:scale-95 cursor-pointer w-full sm:max-w-[340px] group">
            ...
            {/* Contenedor fijo con fondo para igualar tamaños */}
